...
void Pause();                                 // 暂停播放
void Resume();                                // 继续播放
void SeekTo(TimeSpan position);               // 跳转到指定位置
void SetPlaybackMode(PlaybackMode mode);      // 设置播放模式（顺序、随机、单曲）
void SetPlaybackSpeed(double speed);          // 设置播放速度（1.0x, 1.5x等）
void SetPlaylist(List<Song> playlist);        // 设置当前播放列表
List<Song> AddToPlaylist(Song song);                // 向播放队列中添加歌曲
List<Song> RemoveFromPlaylist(Song song);           // 从播放队列中移除歌曲
List<Song> GetCurrentPlaylist();              // 获取当前播放队列
Lyrics GetLyricsForSong(Song song);           // 获取指定歌曲的歌词
void DisplayLyrics(Lyrics lyrics);            // 显示歌词并滚动高亮
void SetLyricsMode(LyricsMode mode);          // 设置歌词模式（普通/纯净）
...
```csharp
bool ImportSongs(string folderPath);                         // 管理员导入歌曲（本地路径）
bool ExportSongs(string exportPath);                         // 导出歌曲到本地路径
List<Song> GetAllSongs();                                    // 获取所有歌曲
List<Song> FilterSongsByTag(string tag);                     // 通过标签筛选歌曲
List<Song> FilterSongsByTag(List<string>tags);               // 通过多个标签筛选歌曲（重载）
bool AssignTagToSong(Song song, string tag);                 // 为歌曲添加标签

void CreateSonglist(string name);                            // 创建空歌单
...
string GetShareLink(Songlist songlist)                       // 生成歌单分享链接
```
...
| **曲库管理** (导入导出歌曲、歌单管理、打标签) | `Services/LibraryService.cs`  | 本地曲库、标签、歌单管理功能。                               |
